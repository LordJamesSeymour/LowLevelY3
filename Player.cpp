...
void Player::moveHorizontal(float deltaTime, Level& level, unsigned int windowWidth)
{
    ...
    if (bounds.position.x > static_cast<float>(windowWidth))
    {
        m_sprite->setPosition({ -bounds.size.x, m_sprite->getPosition().y });
    }
    else if (bounds.position.x + bounds.size.x < 0.f)
    {
        m_sprite->setPosition({ static_cast<float>(windowWidth), m_sprite->getPosition().y });
    }
}
