...
bool Level::loadFromFile(const std::string& mapPath,
...
bool Level::isSolidTile(int col, int row) const
{
    if (!isInside(col, row))
        return false;
...
bool Level::isBreakTile(int col, int row) const
{
    if (!isInside(col, row))
        return false;
...
char Level::getTile(int col, int row) const
{
    if (!isInside(col, row))
        return 'O';
...
void Level::breakTile(int col, int row)
{
    if (!isInside(col, row))
        return;
...
void Level::spawnRandomBreakBlocks(int count, const sf::FloatRect& forbiddenArea)
{
...
const std::string& Level::getLastError() const
{
    return m_lastError;
}

bool Level::isInside(int col, int row) const
{
