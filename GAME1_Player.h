...
// Handles movement, jumping, gravity, collision, and breakable blocks.
class GAME1_Player
{
public:
	bool load(const std::string& texturePath, sf::Vector2f startPosition);
	void update(float deltaTime, GAME1_Level& level, unsigned int windowWidth);
...
private:
	...
	void moveHorizontal(float deltaTime, GAME1_Level& level, unsigned int windowWidth);
